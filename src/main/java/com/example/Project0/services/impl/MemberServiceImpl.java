...
public class MemberServiceImpl implements MemberService {
    ...
    public boolean login(LogInDTO logInDTO) {
        ...
        // 정보가 비어있으면 로그인시도를 한 email을 가진 데이터 자체가 없는 정보라는 뜻이다
    ...
    public MemberDetailDTO findById(Long memberId) {
        // findById를 하게 되면 jpa에서 Optional타입으로 준다
        Optional<MemberEntity> memberEntityOptional = memberRepository.findById(memberId);
        // Optional 타입을 Entity타입으로 바꾸기 위해 .get()을 붙인다
    ...
    @Override
    public boolean checkEmailDuplicate(String signupEmail) {
        return memberRepository.existsByMemberEmail(signupEmail);
    }
}
